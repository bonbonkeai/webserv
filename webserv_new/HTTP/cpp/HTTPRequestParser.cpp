...
static bool containsBareLF(const std::string& s)
{
...
// 只解码 %2e/%2E 与 %2f/%2F，用于防路径穿越
static bool decodeDotSlashPercent(const std::string& in, std::string& out)
{
    out.clear();
...
HTTPRequestParser::HTTPRequestParser(const HTTPRequestParser& copy) : 
                    ...
                    _received_any(copy._received_any),
...
void HTTPRequestParser::resetForNextRequest()
{
    ...
    _chunk_waiting_size = true;
    _chunk_expected_size = 0;
    _received_any = !_buffer.empty();
}

void HTTPRequestParser::setMaxBodySize(std::size_t max)
{
    _req.max_body_size = max;
}
...
bool HTTPRequestParser::hasReceivedData() const
{
    return _received_any;
}
...
bool	HTTPRequestParser::splitUri()
{
    ...
    std::string decoded;
    if (!decodeDotSlashPercent(_req.path, decoded))
        return (fail(400));
    _req.path = decoded;
...
bool	HTTPRequestParser::parseRequestLine()
{
	...
	if (_req.method.empty() || _req.uri.empty() || _req.version.empty())
		return (fail(400));
    if (_req.version.compare(0, 5, "HTTP/") != 0)
        return (fail(400));
