...
HTTPResponse PostRequest::handleRawUploadFallback(const EffectiveConfig& cfg)
{
    std::string upload_root = cfg.root.empty() ? "./www" : cfg.root;
    const std::string UPLOAD_DIR = FileUtils::joinPath(upload_root, "upload");
...
HTTPResponse PostRequest::handle(const EffectiveConfig& cfg)
{
    ...
    return handleRawUploadFallback(cfg);
}
