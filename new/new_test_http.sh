...
start_server() {
    ...
    echo "cmd: $SERVER_BIN ${SERVER_ARGS:-config_defaut/default.conf}"

    if [ -n "$SERVER_ARGS" ]; then
        "$SERVER_BIN" $SERVER_ARGS >/tmp/test_http_${port}.log 2>&1 &
    else
        "$SERVER_BIN" config_defaut/default.conf >/tmp/test_http_${port}.log 2>&1 &
    fi
    SERVER_PID=$!
...
nc_raw() {
    ...
    # 使用timeout，同时捕获stdout和stderr
    printf "%b" "$payload" | timeout 5 nc -v "$HOST" "$port" > "$temp_file" 2>&1
    local exit_code=$?
...
count_http_responses() {
    local text="$1"
    echo "$text" | grep -o 'HTTP/1\.1 [0-9][0-9][0-9]' | wc -l
}
...
nc_raw_improved() {
    ...
    # 使用printf发送精确的字节
    printf "%b" "$payload" | timeout "$timeout" nc -v "$HOST" "$port" 2>&1
    local exit_code=$?
...
test_missing_host() {
    ...
    # 测试1：简单的缺失Host请求
    echo "测试1: 缺失Host (无其他头)"
    local out1
    out1="$(nc_raw "$port" 'GET /hello.txt HTTP/1.1\r\n\r\n')"
    local exit1=$?
    ...
    # 检查响应
    if echo "$out1" | grep -q '^HTTP/1\.1 400 '; then
        print_ok "缺失Host请求返回400 (严格模式) on $port"
    elif echo "$out1" | grep -q '^HTTP/1\.1 200 '; then
        # 如果返回200，检查是否使用了默认Host
        print_ok "缺失Host请求返回200 (宽容模式，可能使用默认Host) on $port"
        echo "注意: 服务器在没有Host头的情况下返回200，这可能是实现的选择"
    elif echo "$out1" | grep -q 'TIMEOUT'; then
        print_ok "缺失Host请求超时 (服务器在等待更多数据) on $port"
    elif echo "$out1" | grep -q '^HTTP/1\.1 '; then
        local status=$(echo "$out1" | grep '^HTTP/1\.1 ' | head -1 | cut -d' ' -f2)
        print_ok "缺失Host请求返回状态 $status on $port"
    else
        print_fail "缺失Host请求处理" "没有收到有效的HTTP响应"
        echo "---- 原始输出 ----"
        echo "$out1"
    fi

    # 测试2：缺失Host但有Connection: close
    echo
    echo "测试2: 缺失Host + Connection: close"
    local out2
    out2="$(nc_raw_improved "$port" 'GET /hello.txt HTTP/1.1\r\nConnection: close\r\n\r\n')"
    
    echo "输出:"
    echo "$out2"
    
    if echo "$out2" | grep -q '^HTTP/1\.1 400 '; then
        print_ok "缺失Host + Connection: close 返回400 on $port"
    else
        print_fail "缺失Host + Connection: close 应该返回400" "但返回了其他响应"
        echo "$out2"
    fi
}
...
test_bare_lf_only() {
    local port="$1"
    section "3.5.6 行尾没有 CRLF / 出现裸 LF"

    echo "cmd:"
    printf 'GET / HTTP/1.1\nHost: example.local\n\n'

    local out
    out="$(nc_raw "$port" 'GET / HTTP/1.1\nHost: example.local\n\n')"

    # 检查是否崩溃
    if [ -z "$out" ]; then
        print_fail "bare LF request handling on $port" "no response (server may have crashed)"
        return
    fi
    ...
    if echo "$out" | grep -q '^HTTP/1\.1 '; then
        local status
        status="$(echo "$out" | grep '^HTTP/1\.1 ' | head -1 | cut -d' ' -f2)"
        
        case "$status" in
            200)
                print_ok "bare LF accepted with 200 (lenient implementation) on $port"
                ;;
            400)
                print_ok "bare LF rejected with 400 (strict implementation) on $port"
                ;;
            *)
                print_ok "bare LF handled with status $status (acceptable) on $port"
                ;;
        esac
        
        # 检查响应是否完整（有Content-Length或Transfer-Encoding）
        if echo "$out" | grep -qi '^content-length:' || echo "$out" | grep -qi '^transfer-encoding:'; then
            : # 有framing信息，good
        else
            echo "warning: response missing framing headers on $port"
        fi
    else
        print_fail "bare LF request handling on $port" "no valid HTTP response received"
        echo "---- nc output ----"
        echo "$out"
    fi
}
...
s = socket.create_connection(('$HOST', $port))
s.send(b'POST /upload/chunk_short.txt HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n5\r\nhel')
time.sleep(1)
s.settimeout(0.5)
try:
    resp = s.recv(1024)
    if b'201' in resp:
        print('FAIL: short chunk succeeded')
    else:
        print('OK: short chunk rejected')
except socket.timeout:
    print('OK: server waits for complete chunk')
" | grep -q 'OK' && print_ok "short chunk handled correctly" || print_fail "short chunk handling incorrect"
    
    # 测试长数据（应该拒绝）
    python3 -c "
import socket, time, sys
s = socket.create_connection(('$HOST', $port))
s.send(b'POST /upload/chunk_long.txt HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n5\r\nhelloo\r\n0\r\n\r\n')
s.settimeout(1)
resp = s.recv(1024).decode()
if '400' in resp or '413' in resp:
    print('OK: long chunk rejected')
else:
    print('FAIL: unexpected response')
" | grep -q 'OK' && print_ok "long chunk rejected" || print_fail "long chunk should be rejected"
}
...
import socket, time, sys
s = socket.create_connection(('$HOST', $port))
s.send(b'POST /upload/chunk_nocrlf.txt HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n5\r\nhello0\r\n\r\n')
time.sleep(0.5)
s.settimeout(0.5)
try:
    resp = s.recv(1024)
    if b'400' in resp:
        print('OK: missing CRLF rejected')
    else:
        print('FAIL: unexpected response')
except socket.timeout:
    print('FAIL: server hung')
" | grep -q 'OK' && print_ok "missing CRLF handled" || print_fail "missing CRLF handling"
}
...
test_get_directory_autoindex() {
    ...
    # 接受200（开启目录列表）或403（关闭目录列表）
    if [ "$status" = "200" ] || [ "$status" = "403" ]; then
        print_ok "GET /dir/ without index returns $status on $port (acceptable)"
        ...
        if [ "$status" = "200" ]; then
            ...
            # 403状态，检查是否有错误信息
            local body_sz
            body_sz="$(body_size_of_file /tmp/b_dir_auto.$$)"
            if [ "$body_sz" -gt 0 ]; then
                print_ok "403 response includes error body on $port"
            fi
        fi
    else
        print_fail "GET /dir/ without index returns acceptable status" "expected 200 or 403, got $status"
        echo "---- headers ----"
        cat /tmp/h_dir_auto.$$ 2>/dev/null || true
        echo "---- body ----"
        cat /tmp/b_dir_auto.$$ 2>/dev/null || true
    ...
    # 通用检查：必须有framing
    if grep -iq '^content-length:' /tmp/h_dir_auto.$$ || grep -iq '^transfer-encoding:[[:space:]]*chunked' /tmp/h_dir_auto.$$; then
        print_ok "response has framing header on $port"
    else
        print_fail "response has framing header on $port" "missing both Content-Length and Transfer-Encoding"
    fi
}
...
test_get_basic_matrix() {
    ...
    for p in "/hello.txt" "/nope.txt" "/dir/" "/emptydir/"; do
        echo "cmd: curl -is --http1.1 \"$base$p\" | sed -n '1,30p'"
        status="$(curl -sS -D "$temp_headers" "$base$p" --http1.1 -o "$temp_body" -w '%{http_code}' 2>/dev/null || true)"

        case "$p" in
            "/hello.txt")
                check_status "$p" "$status" "200" "200"
                ;;
            "/nope.txt")
                check_status "$p" "$status" "404" "404"
                ;;
            "/dir/")
                # 非空目录，期望200（目录列表）或403（禁止列表）
                check_status "$p" "$status" "200" "200|403"
                ;;
            "/emptydir/")
                # 空目录可以返回200（空列表）、403（禁止）或404（不存在）
                check_status "$p" "$status" "200|403|404" "200/403/404"
                ;;
        esac

        # 检查framing
        if grep -iq '^content-length:' "$temp_headers" || grep -iq '^transfer-encoding:[[:space:]]*chunked' "$temp_headers"; then
            print_ok "matrix $p has framing header on $port"
        else
            print_fail "matrix $p has framing header on $port" "missing both Content-Length and Transfer-Encoding"
            echo "---- headers ----"
        ...
        # 检查Content-Type（对于错误页面也应有Content-Type）
        if grep -iq '^content-type:' "$temp_headers"; then
            print_ok "matrix $p has Content-Type on $port"
        else
            print_fail "matrix $p has Content-Type on $port" "missing Content-Type"
            echo "---- headers ----"
...
check_status() {
    ...
    if echo "$got" | grep -Eq "^($expected_pattern)\$"; then
        print_ok "matrix $path -> $got on $port (acceptable)"
    else
        print_fail "matrix $path returns acceptable status" "got $got, expected $expected_desc"
    fi
}
...
#
    ...
    #prepare_http_method_get_fixtures
    #mkdir -p "$UPLOAD_DIR"
    #echo "HELLO WORLD" > "$UPLOAD_DIR/hello.txt"
